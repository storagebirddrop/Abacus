  ...
  return (
    <div className="page-surface p-8 max-w-3xl">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        {/* Blockchain Sync */}
        <section>
          <h2 className="text-base font-semibold text-foreground mb-4">Blockchain Sync</h2>
          ...
          <label className="flex items-center gap-3 mb-6 cursor-pointer">
            ...
            />
            <div>
              <span className="text-sm font-medium text-foreground">Enable blockchain sync</span>
          ...
          {settings.sync_enabled && (
            <div className="space-y-6 pl-7">
              {/* Privacy warning */}
              <div className="rounded border border-amber-800/50 bg-amber-950/30 light:border-amber-300 light:bg-amber-50 px-3 py-2">
                <p className="text-xs text-amber-500 light:text-amber-800">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-foreground">Esplora API <span className="text-muted-foreground">(public or self-hosted REST)</span></span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">
                      Rate limit <span className="font-normal text-muted-foreground">(ms between requests)</span>
                    </label>
                    <input
                      type="number"
                      min={0}
                      className="w-32 border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
      ...
      <form onSubmit={saveToken} className="mt-10 border-t border-border pt-8 space-y-3">
        ...
        <div>
          <label htmlFor="api-token" className="block text-xs font-medium text-muted-foreground mb-1">
            API token
          </label>
          <input
            id="api-token"
            type="password"
            autoComplete="off"
            className="w-full sm:w-96 border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            value={apiToken}
        ...
        <div className="flex items-center gap-4">
          ...
          {tokenSaved && <span className="text-sm text-success">Saved</span>}
        </div>
      </form>
    </div>
  )
